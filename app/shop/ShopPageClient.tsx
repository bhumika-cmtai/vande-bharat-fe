...
const allFilterGroups = [
    { 
        ...
        options: [ 
            ...
            { id: 'WELLNESS_PRODUCT', label: 'Wellness' }, 
            { id: 'FOOD_PRODUCT', label: 'Food & Beverages' }, 
        ] 
    }, 
    { 
        id: 'sub_category', 
...
export default function ShopPageClient() {
  ...
  const getInitialPriceRange = useCallback(() => {
    return {
  ...
  // Initialize state with URL params immediately
  const [selectedFilters, setSelectedFilters] = useState<Record<string, string[]>>(getInitialFilters);
  const [currentPriceRange, setCurrentPriceRange] = useState(getInitialPriceRange);
  const [isInitialized, setIsInitialized] = useState(false);

  // Single useEffect for initialization and URL changes
  useEffect(() => {
    // Update filters from URL
    const filtersFromURL = getInitialFilters();
    const priceRangeFromURL = getInitialPriceRange();
    
    setSelectedFilters(filtersFromURL);
    setCurrentPriceRange(priceRangeFromURL);

    // Fetch products with URL parameters immediately
    const paramsForFetch = Object.fromEntries(searchParams.entries());
    dispatch(fetchProducts(paramsForFetch));
    
    setIsInitialized(true);
  }, [searchParams, dispatch, getInitialFilters, getInitialPriceRange]);
  ...
  // Don't render until initialized to prevent flash of unfiltered content
  if (!isInitialized) {
    return (
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-gray-600">
                Showing <span className='font-bold'>{products.length}</span> of <span className='font-bold'>{totalProducts}</span> products
              </p>
            </div>
